...
const SAMPLE_DB = [
  { id: 1, name: "Lemon Herb Chicken Bowl", calories: 548, protein: 41, carbs: 38, fat: 14, flavor: "Mediterranean", steps: ["Grill 200g chicken breast seasoned with lemon and herbs for 6 mins each side.", "Cook 80g quinoa in vegetable stock for 12 minutes.", "Slice cucumber, halve cherry tomatoes, crumble 30g feta.", "Assemble bowl, drizzle with olive oil and lemon juice."] },
  { id: 2, name: "Spicy Beef & Rice Power Bowl", calories: 562, protein: 43, carbs: 42, fat: 15, flavor: "Spicy", steps: ["Brown 180g lean beef mince with garlic and chilli flakes.", "Cook 90g brown rice according to packet instructions.", "Drain and rinse 80g black beans, warm in pan.", "Assemble bowl with salsa and fresh coriander on top."] },
  { id: 3, name: "Turkey Meatball Pasta", calories: 530, protein: 39, carbs: 55, fat: 12, flavor: "Comfort Food", steps: ["Mix 200g turkey mince with garlic, form into 8 meatballs.", "Fry meatballs in olive oil for 10 mins until golden.", "Cook 90g whole wheat pasta, warm 150ml marinara sauce.", "Combine everything and top with 20g grated parmesan."] },
];
...
function RecipeCard({ recipe, flavor }) {
  const [expanded, setExpanded] = useState(false);
  const imageUrl = getFallbackImage(recipe.flavor || flavor);
  return (
    <div style={{ background: "linear-gradient(160deg,#1a1a2e,#16213e)", borderRadius: 20, overflow: "hidden", border: "1px solid #2a2a4a", boxShadow: "0 20px 60px rgba(0,0,0,0.5)", marginBottom: 24 }}>
      <div style={{ position: "relative", height: 200, overflow: "hidden" }}>
        <img src={imageUrl} alt={recipe.name} style={{ width: "100%", height: "100%", objectFit: "cover", filter: "brightness(0.7)" }} />
...
export default function Home() {
  ...
  async function handleGenerate(surpriseMe = false) {
    setError(null); setLoading(true); setRecipes([]); setPath(null);
    try {
      if (!surpriseMe) {
        const matches = SAMPLE_DB.filter(m => matchesMacros(m, { calories, protein }));
        if (matches.length > 0) { setPath("DATABASE"); setRecipes(matches); setLoading(false); return; }
      }
      setPath("AI");
      const recipe = await callClaudeAPI(calories, protein, ingredient || "chicken", flavor);
      setRecipes([recipe]);
    } catch (e) {
      setError("Something went wrong. Please try again."); console.error(e);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "radial-gradient(ellipse at top left,#0f0c29,#302b63,#24243e)", padding: "0 0 60px" }}>
      <div style={{ textAlign: "center", padding: "50px 20px 30px" }}>
        ...
        <h1 style={{ fontSize: "clamp(28px,5vw,50px)", fontFamily: "Georgia, serif", color: "#fff", margin: 0, lineHeight: 1.1 }}>Macro Meal<br /><span style={{ color: "#4ecdc4" }}>Generator</span></h1>
        <p style={{ color: "#888", marginTop: 12, fontSize: 14, maxWidth: 380, margin: "12px auto 0" }}>Enter your targets. Get a perfectly matched meal — from your library or invented just for you.</p>
      </div>
      ...
      <div style={{ maxWidth: 460, margin: "0 auto", padding: "0 16px" }}>
        <div style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", borderRadius: 24, padding: 26, border: "1px solid rgba(255,255,255,0.1)", boxShadow: "0 30px 80px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#ccc", marginBottom: 7 }}>🔥 Calories: <span style={{ color: "#ff6b6b", fontWeight: 700 }}>{calories} kcal</span></label>
            <input type="range" min={300} max={900} value={calories} onChange={e => setCalories(+e.target.value)} style={{ width: "100%", accentColor: "#ff6b6b" }} />
          </div>

          <div style={{ marginBottom: 18 }}>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#ccc", marginBottom: 7 }}>💪 Protein: <span style={{ color: "#4ecdc4", fontWeight: 700 }}>{protein}g</span></label>
        ...
        <div style={{ marginTop: 18 }}>
          {recipes.map((recipe, i) => <RecipeCard key={i} recipe={recipe} flavor={flavor} />)}
        </div>
